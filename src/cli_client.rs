...
fn deny_source_note(tag: &str) -> Option<&'static str> {
    match tag {
        ...
        "validation" => Some(
            "rejected by request or authority validation; correct the request or use the stated typed authority",
        ),
        _ => None,
    }
}
...
pub(crate) async fn handle_status(socket: Option<String>, json: bool) -> Result<()> {
    ...
    let ping = match client.send_admin(server::AdminRequest::Ping).await {
        ...
        Ok(server::AdminResponse::Error { message }) => {
            if json {
                exit_unreachable_status_json(&client, "ping_refused", message);
            }
        ...
        Ok(_) => {
            if json {
                exit_unreachable_status_json(
                    &client,
                    "unexpected_ping_response",
                    "server returned an unexpected response to ping".to_string(),
                );
            }
            eprintln!("Server: unexpected ping response");
            std::process::exit(1);
        }
        Err(e) => {
            let e = describe_connect_failure(e, &client, source);
            if json {
                exit_unreachable_status_json(&client, "unreachable", format!("{e:#}"));
            }
    ...
    match client.send_admin(server::AdminRequest::Status).await {
        ...
        Ok(server::AdminResponse::Error { message })
            if !client.has_admin_token() && message.contains("operator authority") =>
        {
            if json {
                return print_json(&serde_json::json!({
                    "schema_version": JSON_SCHEMA_VERSION,
        ...
        Ok(server::AdminResponse::Error { message }) => {
            ...
            eprintln!("Server: status request failed: {message}");
            std::process::exit(1);
        }
        Ok(_) => {
            if json {
                exit_reachable_status_json(
                    ...
                    "server returned an unexpected response to status".to_string(),
                );
            }
            eprintln!("Server: unexpected status response");
            std::process::exit(1);
        }
        Err(e) => {
            if json {
...
fn exit_unreachable_status_json(
    client: &daemon_client::Client,
    code: &'static str,
    message: String,
) -> ! {
