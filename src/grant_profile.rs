...
//! type into `guard session new`: a ttl, allow/deny globs, and evaluator prompt
//! context. `guard session new --profile <name>` mints a session token from the
...
fn validate_profile(profile: &GrantProfile) -> Result<()> {
    ...
    if profile.allow.is_empty() && profile.deny.is_empty() && profile.prompt_append.is_none() {
        bail!(
            "session profile '{}' grants nothing: set at least one of allow, deny, or prompt_append",
            profile.name
        );
    }
    Ok(())
