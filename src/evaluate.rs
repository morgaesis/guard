...
impl Evaluator {
    ...
    async fn one_attempt(
        ...
        let body = if use_function_calling {
            build_function_call_body(api_url, model, system_prompt, command)
        } else {
            build_json_response_body(api_url, model, system_prompt, command)
        };
...
/// Parse a provider response in the preferred mode, then fall back to the other
/// valid OpenAI-compatible shape. Some routers/models ignore request mode and
/// return a tool call for a JSON request, or content for a tool-call request.
...
fn response_shape_summary(parsed: &serde_json::Value) -> String {
    ...
    let error_message = parsed
        ...
        .unwrap_or_else(|| "<none>".to_string());
    format!(
        "response_shape=top_keys:[{}] choices:{} finish_reason:{} message_keys:[{}] content:{} tool_calls:{} error_code:{} error_type:{} error_message:{}",
        top_keys,
        choice_count,
        finish_reason,
        message_keys,
...
mod tests {
    ...
    fn test_chat_bodies_use_reasoning_compatible_token_budget() {
        let tool_body = build_function_call_body(DEFAULT_API_URL, "model", "system", "id");
        let json_body = build_json_response_body(DEFAULT_API_URL, "model", "system", "id");
    ...
    async fn test_tool_choice_client_error_switches_to_json_format() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        let unsupported = r#"{"error":{"message":"No endpoints found that support the provided 'tool_choice' value"}}"#.to_string();
        let good_body = r#"{"choices":[{"message":{"content":"{\"decision\":\"APPROVE\",\"reason\":\"ok\",\"risk\":1}"}}]}"#.to_string();
