...
pub const DEFAULT_TOOLS: &[(&str, &str)] = &[
    ...
    ("helm", "Helm CLI"),
    ("ansible", "Ansible ad-hoc command runner"),
    ...
    fn render_shim(&self, tool: &str) -> String {
        #[cfg(windows)]
        {
            ...
            render_windows_cmd_forwarder(
                &[
                    ...
                    format!("Do not edit manually - regenerate with: guard shim {tool}"),
                ],
                &fixed_args,
            )
        }
        #[cfg(not(windows))]
        {
            ...
            format!(
             ...
             # Do not edit manually - regenerate with: guard shim {tool}\n\
             exec {guard_bin} run '{tool}' \"$@\"\n",
                tool = tool,
                guard_bin = guard_bin
            )
        }
...
mod tests {
    ...
    fn test_render_shim_content() {
        ...
        #[cfg(not(windows))]
        {
            ...
            // Check exec line
            assert!(content.contains("exec '/usr/bin/guard' run 'ssh' \"$@\""));
        }
        #[cfg(windows)]
        {
            assert!(content.starts_with("@echo off"));
