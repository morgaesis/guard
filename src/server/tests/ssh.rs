...
use crate::server::execute::{execute_command, missing_authorized_execution_profile_reason};
...
#[test]
fn safe_allow_rejects_fixed_ssh_diagnostic_without_a_profile() {
    let (cfg, _buf) = make_test_config();
    let reason = deterministic_safe_allow_reason(&cfg, "ssh", &args(&["host01", "id"]));
    assert!(reason.is_none(), "SSH has no executable authority profile");
}

#[test]
fn safe_allow_rejects_vetted_ssh_option_without_a_profile() {
    let (cfg, _buf) = make_test_config();
...
#[tokio::test]
async fn explicit_access_overlay_cannot_authorize_unprofiled_ssh() {
...
"#,
    ...
    assert!(baseline.reason.contains("default-deny"));
    assert!(missing_authorized_execution_profile_reason("ssh").is_some());
    ...
    let explicit = execute_command(explicit_request, &cfg, &caller)
        .await
        .into_response();
    assert!(!explicit.allowed, "SSH remains unprofiled: {explicit:?}");
    assert_eq!(
        cfg.state
            .sessions
            .read()
...
#[test]
fn accept_new_hostkey_does_not_authorize_ssh() {
    let (cfg, _buf) = make_test_config();
    let mut req = ssh_request(Some(SshHostKeyMode::AcceptNew), &["host01", "id"]);
    req.apply_ssh_hostkey_options();
...
#[test]
fn accept_all_hostkey_fails_legacy_classifier() {
    // accept-all injects StrictHostKeyChecking=no, which the option
    // allow-list rejects, so even a fixed diagnostic fails the
    // legacy classifier over an unauthenticated channel.
    let (cfg, _buf) = make_test_config();
    let mut req = ssh_request(Some(SshHostKeyMode::AcceptAll), &["host01", "id"]);
    req.apply_ssh_hostkey_options();
    assert!(deterministic_safe_allow_reason(&cfg, "ssh", &req.args).is_none());
...
fn ssh_options_allow_list_permits_only_vetted_options() {
    ...
    for ok in [
        ...
        // fail the legacy option classifier.
        &[
            "-o",
            "StrictHostKeyChecking=accept-new",
            "-o",
        ...
        // command, so this form shares the same classification as pre-destination
        // options.
    ...
    // Every unvetted option fails the legacy classifier. This covers the
    ...
    for bad in [
        ...
        // Combined short flags are not decomposed; reject conservatively.
        &["-Cq", "host01", "id"][..],
        // A value option with no following value is malformed; reject it.
        &["-p"][..],
        &["host01", "-l"][..],
    ] {
        assert!(
            !ssh_options_all_readonly_safe(&args(bad)),
            "option should fail the legacy classifier: {bad:?}"
        );
    }
}
...
fn ssh_options_reject_dangerous_option_between_host_and_command() {
    ...
    // in that position must fail the classifier.
    for bad in [
        ...
        assert!(
            !ssh_options_all_readonly_safe(&args(bad)),
            "option between host and command must fail: {bad:?}"
        );
    }
    ...
    // not affect classification of the fixed command itself.
...
fn ssh_option_terminator_preserves_remote_command_boundary() {
    let (cfg, _buf) = make_test_config();
    assert!(deterministic_safe_allow_reason(&cfg, "ssh", &args(&["--", "host01", "id"])).is_none());
    assert!(deterministic_safe_allow_reason(&cfg, "ssh", &args(&["host01", "--", "id"])).is_none());
...
fn ssh_o_directive_rejects_newline_smuggled_second_directive() {
    ...
    // The same keyword without a newline passes the legacy classifier.
    assert!(ssh_o_directive_readonly_safe("ConnectTimeout=5"));
}

#[test]
fn ssh_o_stricthostkeychecking_permits_only_secure_values() {
    // Security-preserving values pass the legacy classifier (accept-new is what
    // the --hostkey mode injects).
    assert!(ssh_o_directive_readonly_safe("StrictHostKeyChecking=yes"));
    assert!(ssh_o_directive_readonly_safe(
        "StrictHostKeyChecking=accept-new"
    ));
    // Disabling or deferring host-key verification fails the
    // classifier rather than treating an unauthenticated channel as safe.
    for weak in [
        "StrictHostKeyChecking=no",
        ...
        assert!(
            ...
            "{weak} should fail the legacy classifier"
        );
    }
    // The whole invocation also fails when the caller disables it.
    let (cfg, _buf) = make_test_config();
    assert!(deterministic_safe_allow_reason(
        &cfg,
        "ssh",
...
fn safe_allow_rejects_ssh_forwarding_and_proxy() {
    ...
    for reject in [
        ...
        assert!(
            ...
            "ssh with {reject:?} must fail the legacy classifier"
        );
    }
    // A benign, vetted option still cannot authorize an unprofiled executable.
    assert!(deterministic_safe_allow_reason(
        &cfg,
        "ssh",
