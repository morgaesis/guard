...
|---|---|---|---|
| Agent to daemon wire | Local process submitting requests or impersonating the operator | Execution authority and admin RPCs | Unix peer-credential uid and Windows named-pipe SID authentication, failing closed when the peer identity cannot be resolved; optional `--users` uid allow-list; sockets created `0600`, widened to `0660` only after `--socket-group` assignment; TCP bearer tokens compared in constant time; Unix and TCP admin RPCs require the separate admin bearer, while Windows uses the daemon service SID or local SYSTEM |
