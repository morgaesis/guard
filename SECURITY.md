...
|---|---|---|---|
| Agent to daemon wire | Local process submitting requests or impersonating the operator | Execution authority and admin RPCs | Unix peer-credential uid and Windows named-pipe SID authentication, failing closed when the peer identity cannot be resolved; Windows clients request identification-level pipe security to prevent server impersonation; optional `--users` uid allow-list; sockets created `0600`, widened to `0660` only after `--socket-group` assignment; TCP bearer tokens compared in constant time; Unix and foreground Windows admin RPCs require the separate admin bearer; the packaged Windows service rejects that bearer and accepts only kernel-authenticated local SYSTEM on its named pipe |
