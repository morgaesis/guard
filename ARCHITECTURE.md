...
`--api-endpoints PATH` defines multiple named listeners in one daemon. Each endpoint owns its protocol, upstream, credential reference, policy, client output, and listener mode. Registry and persisted revert plans use the endpoint name, so concurrent listeners using the same protocol cannot cross credentials during rollback. A readonly listener baseline denies writes without a live prompt-bearing session. A session-scoped write routes through the evaluator under that session intent, including when the endpoint policy otherwise allows it. Protocol hard-denies and explicit policy denies remain absolute.

Clients attribute proxied traffic with a standard `Authorization: Bearer <Guard session>` header. `X-Guard-Session` is a compatibility alias. The proxy rejects duplicate or conflicting credentials, strips either accepted form, validates the token against the live session registry, and adds only its fingerprint and prompt context to the stable evaluator request. It revalidates the same live, unsuspended session immediately before forwarding, and binds the fingerprint to held requests, provisionals, and create/delete provenance. A brokered kubeconfig can contain this Guard session bearer while remaining free of every upstream credential; forwarding always removes the client Authorization header before injecting the endpoint's upstream credential. Generated API verb coverage is keyed by endpoint, session fingerprint, typed request fields, namespace, and value-free body shape. Evaluator-generated cells carry provenance, regime, and expiry metadata; stale cells route back to evaluation. Value-bearing mutations remain evaluator-routed until coverage has field-aware value constraints. A session evaluates past a matching global generated deny, while operator-authored coverage and policy denies remain floors. `guard verb coverage list|clear` provides operator inspection and reset. Session history records the API operation and a safe endpoint credential reference without persisting credential values.

The API judge admits evaluator calls through a global concurrency semaphore and endpoint/session-partitioned token buckets and consecutive-error circuits. Unattributed traffic cannot consume the reserved session concurrency slot. A rejected call fails closed through the evaluator error path. Audit events report cumulative attempted, admitted, limited, error, and circuit counters. These controls bound evaluator spend and failure amplification without weakening the consequence floor.

Containment rests on the daemon holding the only upstream credential. For Kubernetes the daemon reads the real bearer token or client certificate from its kubeconfig (exec/auth-provider plugins are rejected because the proxy cannot run them and they would let a client mint credentials), and emits a brokered kubeconfig (`--brokered-kubeconfig-out`) that points only at the proxy. The brokered config contains no upstream `client-certificate`/`exec`/`auth-provider`/`password` field and may contain only the expected Guard session bearer as its token. The proxy consumes that bearer and never forwards it. For other protocols the daemon reads a bearer token from an environment variable (`--api-token-env`) or file (`--api-token-file`), never from a command-line value, and `--api-ca-out` writes the proxy's CA certificate so generic clients can trust the TLS termination. With the agent pointed at the proxy and no other upstream credentials reachable, the proxy is the sole path to the upstream. `--api-proxy` refuses to start with `--exec-as-caller`, which would run a child as the caller and let it read caller-owned credentials around the gate, and refuses a non-loopback bind address. A Guard session bearer conveys scope but is not bound to a network client identity, so a reachable listener would offer the daemon's upstream authority to the network.
...
Under `--gate consequence`, a recoverable write the policy allows is wrapped in the auto-revert envelope. The protocol plans the revert as a plain HTTP request: before an update/patch the proxy snapshots the prior object and the revert restores it (Kubernetes: a `PUT` of the prior object with `resourceVersion` stripped so the restore is unconditional); a create's revert deletes the server-named object (Kubernetes: with background cascade, matching the kubectl delete default); a delete the protocol declares faithfully recreatable snapshots the object first and the revert recreates it (Kubernetes: a `POST` of the snapshot with server-owned metadata and `ownerReferences` stripped, since stale owner uids would make garbage collection remove the restore immediately; GitHub: label deletes; Vercel: env-var deletes). A created object is recorded under the creating connection and exact session fingerprint. Contained cleanup never overrides an explicit policy deny. The planned revert is handed to a `GateSink`, which the daemon implements by arming a session-bound `Provisional` in the shared registry. Its persisted HTTP plan binds the endpoint, protocol, canonical target, and a secret-free digest of the selected upstream credential identity. The sweeper executes only through an exact live match and otherwise defers the still-live change for operator handling. Credential-bearing upstream response headers are stripped unless a protocol explicitly permits one. `guard confirm` / `guard provisionals` / `guard revert` apply unchanged.
...
8. **Verb catalog** (optional, opt-in via `--verbs`): an operator-authored, hot-reloaded catalog of typed operations, and the primary structured mechanism for deterministic, LLM-skipping authorization. Each verb fixes a binary and may define an argv template with pattern-validated, anchored parameters. Rendering substitutes each placeholder as exactly one argv element, so parameter and flag injection are structurally impossible. A verb can also define typed coverage cells over required and forbidden argv elements, option values, positional targets, inventory, namespace, and list fanout. Each cell is `preauthorized`, `evaluate`, or `deny`; an unmatched cell has no effect and never denies its complement. A verb declares its consequence class and optional structured rollback. A preauthorized cell requires `trusted: true`, skips the evaluator, and retains the declared consequence routing.
   ...
   Baseline verbs apply without a session. A session grant activates a `baseline: false` verb with `--verb`, and session coverage overlays baseline preauthorization. Baseline `evaluate` and `deny` cells remain requirements across that overlay unless the cell declares an `override_marker` and the operator-issued grant carries that exact marker with `--override-marker`. Automatically promoted verbs cannot mint override markers. Binary allow-lists, credential checks, request validation, and consequence routing run independently of session coverage. Execute responses expose all applicable cells as `verb_matches` plus optional `verb_guidance`; human clients print these details only for held or denied commands.
...
12. **Generated API verb coverage** (`src/gating/api_promotion.rs`; on by default, disable with `--no-api-verb-coverage`): proxied API requests routed to `evaluate` record only real evaluator verdicts into a bounded YAML store keyed by endpoint, session fingerprint, and the exact typed operation and value-free body shape, excluding object name. Repeated eligible allows activate coverage with the maximum observed risk and common reversibility class; value-bearing mutations remain evaluator-routed until coverage carries field-aware constraints. Repeated denies activate deny coverage. Generated cells carry evaluator provenance, regime stamps, and bounded expiry. A generated deny rejects before another evaluator call in its scope; a live session evaluates past a matching global generated deny, while operator-authored denies remain floors. A generated allow reuses the stored verdict only when `rarity=false`; every allow passes through `decide_gate`, so the consequence floor remains authoritative. `guard verb coverage list|clear` is the operator lifecycle surface.
